...
export default {
  ...
  methods: {
    ...
    deleteData() {
      if (this.event.id) {
        this.customerList = this.customerList.filter((item) => item.id != this.event.id);
...
<template>
  <Layout>
              ...
              <BRow class="g-3">
                <BCol xl="6">
                  <div class="search-box">
                    <input type="text" class="form-control search" placeholder="Search for customer, email, phone, status or something..." v-model="searchQuery" />
                ...
                <BCol xl="6">
                  <BRow class="g-3">
                    <BCol sm="4">
                      <div class="">
                        <flat-pickr v-model="date" placeholder="Select date" :config="rangeDateconfig" class="form-control"></flat-pickr>
                      </div>
                    </BCol>
                    <BCol sm="4">
                      <div>
                ...
                <table class="table align-middle" id="customerTable">
                  ...
                  <tbody class="list form-check-all">
                    <tr v-for="(item, index) of resultQuery" :key="index">
                      ...
                      <td class="status">
                        <span class="badge text-uppercase" :class="{ 'bg-success-subtle text-success': item.status == 'Active', 'bg-danger-subtle text-danger': item.status == 'Block' }">
                          {{ item.status }}
                        </span>
                      </td>
                      <td>
                        <ul class="list-inline hstack gap-2 mb-0">
                          <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Edit">
                            <BLink variant="text-primary" class="d-inline-block edit-item-btn" @click="editDetails(item)">
    ...
    <!-- customer modal  -->
    <BModal v-model="addCustomerModal" id="showModal" hide-footer :title="dataEdit ? 'Edit Customer' : 'Add Customer'" header-class="p-3 bg-light" class="v-modal-custom" centered>
      <b-form action="#" id="addform" class="tablelist-form" autocomplete="off">
        ...
        <div class="mb-3">
          ...
          <flat-pickr :config="config" class="form-control" id="joindate" v-model="event.date" :class="{ 'is-invalid': submitted && !event.date }"></flat-pickr>
          <div class="invalid-feedback">Please select a date.</div>
        </div>
        <div class="mb-3">
          <label for="status-field" class="form-label">Status</label>
          <select class="form-control" data-trigger id="statusfield" v-model="event.status" :class="{ 'is-invalid': submitted && !event.status }">
    ...
    <BModal v-model="deleteModal" modal-class="zoomIn" hide-footer no-close-on-backdrop centered>
      <div class="mt-2 text-center">
        <lottie class="avatar-xl" colors="primary:#f7b84b,secondary:#f06548" :options="defaultOptions2" :height="100" :width="100" />
