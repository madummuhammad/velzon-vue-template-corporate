...
import { allTask } from "@/common/data"
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.dataEdit) {
        ...
        if (this.submitted && (this.event.project && this.event.task && this.event.creater && this.event.dueDate && this.event.status && this.event.priority)) {
          ...
          this.allTask = this.allTask.map(item => item.id.toString() == this.event.id.toString() ? { ...item, ...this.event } : item);
        }
      } else {
        this.submitted = true;
        if (this.submitted && (this.event.project && this.event.task && this.event.creater && this.event.dueDate && this.event.status && this.event.priority)) {
    ...
    deleteData() {
      if (this.event.id) {
        this.allTask = this.allTask.filter((item) => item.id != this.event.id);
...
<template>
  <Layout>
              ...
              <BRow class="g-3">
                <BCol xxl="5" sm="12">
                  <div class="search-box">
                    <input type="text" class="form-control search bg-light border-light" placeholder="Search for tasks or something..." v-model="searchQuery" />
                    <i class="ri-search-line search-icon"></i>
                  </div>
                </BCol>

                <BCol xxl="3" sm="4">
                  <flat-pickr v-model="filterdate1" placeholder="Select date" :config="rangeDateconfig" class="form-control bg-light border-light"></flat-pickr>
                </BCol>
                  ...
                  <tr v-for="(task, index) of resultQuery" :key="index">
                        ...
                        <div class="flex-shrink-0 ms-4">
                          <ul class="list-inline tasks-list-menu mb-0">
                            <li class="list-inline-item">
                              <Link href="/apps/tasks-details"><i class="ri-eye-fill align-bottom me-2 text-muted"></i></Link>
                            </li>
                    ...
                    <td class="assignedto">
                      <div class="avatar-group">
                        <BLink href="javascript: void(0);" v-for="(task, index) of task.subItem" :key="index" class="avatar-group-item" data-bs-toggle="tooltip" v-b-tooltip.hover title="Frank">
                          <img :src="task.img" alt="" class="rounded-circle avatar-xxs" />
                        </BLink>
                      </div>
                    </td>
    ...
    <!-- task list modal -->
    <BModal v-model="taskListModal" id="showmodal" modal-class="zoomIn" hide-footer header-class="p-3 bg-info-subtle taskModal" class="v-modal-custom" centered size="lg" :title="dataEdit ? 'Edit Task' : 'Add Task'">
      <b-form id="addform" class="tablelist-form" autocomplete="off">
        <BRow class="g-3">
          ...
          <BCol lg="12">
            ...
            <input type="text" id="projectName" class="form-control" placeholder="Project name" v-model="event.project" :class="{ 'is-invalid': submitted && !event.project }" />
            <div class="invalid-feedback">Please enter a project name.</div>
          </BCol>
          <BCol lg="12">
            <div>
              <label for="tasksTitle-field" class="form-label">Title</label>
              <input type="text" id="tasksTitle" class="form-control" placeholder="Title" v-model="event.task" :class="{ 'is-invalid': submitted && !event.task }" />
              <div class="invalid-feedback">Please enter a title.</div>
            </div>
          </BCol>
          <BCol lg="12">
            <label for="createName-field" class="form-label">Client Name</label>
            <input type="text" id="createName" class="form-control" placeholder="Client name" v-model="event.creater" :class="{ 'is-invalid': submitted && !event.creater }" />
          ...
          <BCol lg="6">
            ...
            <flat-pickr placeholder="Select date" :config="timeConfig" class="form-control flatpickr-input" id="date" v-model="event.dueDate" :class="{ 'is-invalid': submitted && !event.dueDate }"></flat-pickr>
