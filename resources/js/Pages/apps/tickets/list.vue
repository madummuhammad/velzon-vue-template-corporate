...
import { ticketsList } from "@/common/data"
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.dataEdit) {
        ...
        if (this.submitted && (this.event.title && this.event.client && this.event.assigned && this.event.create && this.event.due && this.event.priority && this.event.status)) {
          ...
          this.ticketsList = this.ticketsList.map(item => item.tick_id.toString() == this.event.tick_id.toString() ? { ...item, ...this.event } : item);
        }
      } else {
        this.submitted = true;
        if (this.submitted && (this.event.title && this.event.client && this.event.assigned && this.event.create && this.event.due && this.event.priority && this.event.status)) {
          const data = {
            tick_id: (Math.floor(Math.random() * 100 + 20) - 20),
            id: '#VLZ4' + (Math.floor(Math.random() * 100 + 20) - 20),
            ...this.event
          };
          this.ticketsList = [data, ...this.ticketsList];
          this.modalShow = false;
        }
      }
    },
    ...
    deleteModalToggle(data) {
      ...
      this.event.tick_id = data.tick_id;
    },

    deleteData() {
      if (this.event.tick_id) {
        this.ticketsList = this.ticketsList.filter((item) => item.tick_id != this.event.tick_id);
...
<template>
  <Layout>
              ...
              <BRow class="g-3">
                <BCol xxl="5" sm="12">
                  <div class="search-box">
                    <input type="text" class="form-control search bg-light border-light" placeholder="Search for ticket details or something..." v-model="searchQuery" />
                    <i class="ri-search-line search-icon"></i>
                  </div>
                </BCol>

                <BCol xxl="3" sm="4">
                  <flat-pickr v-model="filterdate1" :config="rangeDateconfig" class="form-control bg-light border-light" placeholder="Select date"></flat-pickr>
                </BCol>

                <BCol xxl="3" sm="4">
                  <div class="input-light">
                    <Multiselect v-model="filtervalue1" :close-on-select="true" :searchable="true" :create-option="true" :options="[
                      { value: '', label: 'Status' },
                      { value: 'All', label: 'All' },
            ...
            <div class="table-responsive table-card mb-4">
                  ...
                  <tr v-for="(data, index) of resultQuery" :key="index">
                    ...
                    <td>
                      <BDropdown toggle-class="btn btn-soft-secondary btn-sm arrow-none" size="sm" no-caret>
                        ...
                        <Link class="dropdown-item" href="/apps/tickets-details"><i class="ri-eye-fill align-bottom me-2 text-muted"></i>View</Link>
                        <BDropdownItem href="#" class="edit-item-btn" @click="editDetails(data)"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</BDropdownItem>
                        <BDropdownItem href="javascript:void(0);" class="remove-item-btn" @click="deleteModalToggle(data)"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</BDropdownItem>
                      </BDropdown>
                    </td>
                  </tr>
                </tbody>
              ...
              <div class="noresult" v-if="resultQuery.length < 1">
                <div class="text-center">
                  <lottie class="avatar-xl" colors="primary:#121331,secondary:#08a88a" :options="defaultOptions" :height="90" :width="90" />
    ...
    <!-- ticket list modal -->
    <BModal v-model="modalShow" id="showModal" modal-class="zoomIn" :title="dataEdit ? 'Edit Ticket' : 'Add Ticket'" title-class="exampleModalLabel" hide-footer header-class="p-3 bg-info-subtle" class="v-modal-custom" size="lg" centered>
        ...
        <BRow class="g-3">
          ...
          <BCol lg="12">
            <div>
              ...
              <input type="text" id="tasksTitle" class="form-control" placeholder="Title" v-model="event.title" :class="{ 'is-invalid': submitted && !event.title }" />
          ...
          <BCol lg="6">
            <div>
              ...
              <input type="text" id="clientName" class="form-control" placeholder="Client Name" v-model="event.client" :class="{ 'is-invalid': submitted && !event.client }" />
          ...
          <BCol lg="6">
            <div>
              ...
              <input type="text" id="assignedtoName" class="form-control" placeholder="Assigned to" v-model="event.assigned" :class="{ 'is-invalid': submitted && !event.assigned }" />
              <div class="invalid-feedback">Please enter a Assignes.</div>
            </div>
          </BCol>
          <BCol lg="6">
            <label for="date-field" class="form-label">Create Date</label>
            <flat-pickr :config="config" placeholder="Create Date" class="form-control" id="cdate" v-model="event.create" :class="{ 'is-invalid': submitted && !event.create }">
            </flat-pickr>
            <div class="invalid-feedback">Please enter a create date.</div>
          </BCol>
          <BCol lg="6">
            <label for="duedate-field" class="form-label">Due Date</label>
            <flat-pickr :config="config" placeholder="Due Date" class="form-control" id="ddate" v-model="event.due" :class="{ 'is-invalid': submitted && !event.due }">
            </flat-pickr>
            <div class="invalid-feedback">Please enter a due date.</div>
          </BCol>
          <BCol lg="6">
            <label for="ticket-status" class="form-label">Status</label>
            <select class="form-control" data-plugin="choices" name="ticket-status" id="ticketstatus" v-model="event.status" :class="{ 'is-invalid': submitted && !event.status }">
          ...
          <BCol lg="6">
            ...
            <select class="form-control" data-plugin="choices" name="priority-field" id="priority" v-model="event.priority" :class="{ 'is-invalid': submitted && !event.priority }">
