...
<template>
  <Layout>
            ...
            <div class="d-md-flex align-items-center">
              ...
              <div class="flex-shrink-0">
                <div class="d-flex gap-1 flex-wrap">
                  ...
                  <BButton class="btn btn-soft-danger" id="remove-actions" @click="deleteMultiple"><i class="ri-delete-bin-2-line"></i></BButton>
                </div>
              </div>
            </div>
          </BCardHeader>
              ...
              <BRow class="g-3">
                <BCol xxl="5" sm="6">
                  <div class="search-box">
                    <input type="text" class="form-control search" placeholder="Search for application ID, company, designation status or something..." v-model="searchQuery">
                ...
                <BCol xxl="2" sm="4">
                  <div>
                    <Multiselect class="form-control" v-model="status1" :close-on-select="true" :searchable="true" placeholder="Select Status" :create-option="true" :options="[
                      { value: 'All', label: 'All' },
                ...
                <BCol xxl="2" sm="4">
                  <div>
                    ...
                    <Multiselect class="form-control" v-model="value1" :close-on-select="true" :searchable="true" :create-option="true" placeholder="Select Type" :options="[
                      { value: 'All', label: 'All' },
                ...
                <BCol xxl="1" sm="4">
                  <div>
                    <BButton type="button" variant="success" @click="SearchData" class="w-100"> <i class="ri-equalizer-fill me-1 align-bottom"></i>
                      Filters
                    </BButton>
                  </div>
                </BCol>
          ...
          <BCardBody class="pt-0">
            <div>
              <ul class="nav nav-tabs nav-tabs-custom nav-success mb-3" role="tablist">
                <li class="nav-item">
                  <BLink class="nav-link  All py-3 active" data-bs-toggle="tab" id="All" href="#" role="tab" aria-selected="true" @click="changecategory('All')">
                    All Application
                  </BLink>
                </li>
                <li class="nav-item">
                  <BLink class="nav-link py-3 New" data-bs-toggle="tab" id="New" href="#" role="tab" aria-selected="false" @click="changecategory('New')">
                    New
                  </BLink>
                </li>
                <li class="nav-item">
                  <BLink class="nav-link py-3 Pending" data-bs-toggle="tab" id="Pending" href="#" role="tab" aria-selected="false" @click="changecategory('Pending')">
                    Pending
                    <BBadge variant="danger" class="align-middle ms-1">2</BBadge>
                  </BLink>
                </li>
                <li class="nav-item">
                  <BLink class="nav-link py-3 Approved" data-bs-toggle="tab" id="Approved" href="#" role="tab" aria-selected="false" @click="changecategory('Approved')">
                    Approved
                  </BLink>
                </li>
                <li class="nav-item">
                  <BLink class="nav-link py-3 Rejected" data-bs-toggle="tab" id="Rejected" href="#" role="tab" aria-selected="false" @click="changecategory('Rejected')">
                    Rejected
                  </BLink>
                </li>
              </ul>
    ...
    <!-- create app modal  -->
    <BModal v-model="createAppModal" id="showModal" :title="dataEdit ? 'Edit Application' : 'Add Application'" header-class="bg-light p-3" title-class="exampleModalLabel" hide-footer class="v-modal-custom" centered>
      <BForm action="#" autocomplete="off" id="addform" class="tablelist-form">
          ...
          <div class="position-relative d-inline-block">
            ...
            <div class="avatar-lg p-1">
              <div class="avatar-title bg-light rounded-circle">
                <img :src="event.img || multi_user_img" id="companylogo-img" class="avatar-md h-auto rounded-circle object-fit-cover" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mb-3">
          ...
          <input type="text" id="company-field" class="form-control" placeholder="Enter company name" required v-model="event.name" :class="{ 'is-invalid': submitted && !event.name }" />
          <div class="invalid-feedback">Please enter a company name.</div>
        </div>

        <div class="mb-3">
          <label for="designation-field" class="form-label">Designation</label>
          <input type="text" id="designation-field" class="form-control" placeholder="Enter designation" required v-model="event.designation" :class="{ 'is-invalid': submitted && !event.designation }" />
          <div class="invalid-feedback">Please enter a designation.</div>
        </div>

        <div class="mb-3">
          <label for="date-field" class="form-label">Apply Date</label>
          <flat-pickr id="date-field" placeholder="Select date" class="form-control flatpickr-input" v-model="event.date" :config="timeConfig" :class="{ 'is-invalid': submitted && !event.date }"></flat-pickr>
          <div class="invalid-feedback">Please select a date.</div>
        </div>

        <div class="mb-3">
          <label for="contact-field" class="form-label">Contacts</label>
          <input type="text" id="contact-field" class="form-control" placeholder="Enter contact" required v-model="event.contacts" :class="{ 'is-invalid': submitted && !event.contacts }" />
        ...
        <BRow class="gy-4 mb-3">
          <BCol md="6">
            <div>
              ...
              <Multiselect placeholder="Select Status" :options="['Approved', 'New', 'Pending', 'Rejected']" v-model="event.status" :class="{ 'is-invalid': submitted && !event.status }" />
          ...
          <BCol md="6">
            <div>
              ...
              <Multiselect placeholder="Select Job Type" :options="['Full Time', 'Part Time']" v-model="event.type" :class="{ 'is-invalid': submitted && !event.type }" />
    ...
    <!-- delete modal-->
    <BModal v-model="deleteModal" body-class="p-5 text-center" modal-class="flip" hide-footer no-close-on-backdrop centered>
      <div class="text-center">
        <div class="text-danger">
          <lottie class="avatar-xl" colors="primary:#405189,secondary:#f06548" :options="defaultOptions1" :height="75" :width="75" />
        </div>
        <div class="mt-4">
