...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.dataEdit) {
        this.submitted = true;
        if (this.submitted && (this.event.name && this.event.company && this.event.email && this.event.phone && this.event.score && this.event.tags)) {
          this.addContactModal = false;
          this.contactsList = this.contactsList.map(item => item.id.toString() == this.event.id.toString() ? { ...item, ...this.event } : item);
        }
      } else {
        this.submitted = true;
        if (this.submitted && (this.event.name && this.event.company && this.event.email && this.event.phone && this.event.score && this.event.tags)) {
          ...
          this.contactsList = [data, ...this.contactsList];
          this.addContactModal = false;
        }
      }
    },
    ...
    deleteData() {
      if (this.event.id) {
        this.contactsList = this.contactsList.filter((item) => item.id != this.event.id);
    ...
    showdetail(data) {
      ...
      document.getElementById('leadscoreno').innerHTML = data.score;
      document.getElementById('tags').innerHTML = '';
      data.tags.forEach((tag) => {
        document.getElementById('tags').innerHTML += '&nbsp;<span class="badge bg-primary-subtle text-primary">' + tag +
          '</span>';
      });
      document.getElementById('lastdate').innerHTML = data.date;
            ...
            <div class="d-flex align-items-center flex-wrap gap-2">
              ...
              <div class="flex-shrink-0">
                <div class="hstack text-nowrap gap-2">
                  ...
                  <BDropdown variant="link" class="card-header-dropdown" toggle-class="btn btn-soft-info arrow-none" menu-class="dropdown-menu-end" :offset="{ alignmentAxis: -105, crossAxis: 0, mainAxis: 10 }">
        ...
        <BCard no-body id="contact-view-detail">
          <BCardBody class="text-center">
            <div class="position-relative d-inline-block">
              <img src="@assets/images/users/avatar-10.jpg" id="image" alt="" class="avatar-lg rounded-circle img-thumbnail">
